...
class CreateAssignmentManager {
    ...
    async loadSampleData() {
        ...
        const mode = urlParams.get('mode') || '평가모드';

        if (competencyCode === 'DIG') {
            try {
                const jsonPath = `task/dig/DIG_${mode}/DIG_${mode}_시나리오.json`;
                ...
                if (scenarioData.tasks && scenarioData.tasks.length > 0) {
                    ...
                    console.log('DIG 시나리오 데이터 로드 완료:', this.tasks.length, '개 과제');
                }
            } catch (error) {
                console.error('DIG 시나리오 데이터 로드 실패:', error);
            }
        }
    }
    ...
    setupEventListeners() {
        ...
        if (addTaskBtn) {
            console.log('과제 추가 버튼 찾음:', addTaskBtn);
            addTaskBtn.addEventListener('click', (e) => {
                console.log('과제 추가 버튼 클릭됨');
                e.preventDefault();
                e.stopPropagation();
                this.addTask();
            });
        } else {
            console.error('과제 추가 버튼을 찾을 수 없음 (addTaskBtn)');
        }
    ...
    addTask() {
        ...
        if (!modal) {
            console.error('taskModal을 찾을 수 없음');
            return;
        }

        console.log('taskModal 찾음, 모달 열기 시도');
        // 모달 폼 초기화
