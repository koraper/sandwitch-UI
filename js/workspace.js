...
class WorkspaceManager {
    ...
    renderCurrentSession() {
        ...
        if (userDisplays.situation) {
            const situationTitle = this.competencyCode === 'PPS' ? '과제 상황' : '비즈니스 상황';
            sessionHtml = `
        ...
        uploadSection.innerHTML = `
            ...
            <div class="accordion-content" id="digDataUploadContent">
                <div class="data-upload-area">
                    ...
                    <button class="btn btn-secondary" id="uploadCsvBtn" onclick="document.getElementById('csvFileInput').click()">
        ...
        analysisSection.innerHTML = `
            ...
            <div class="accordion-content" id="digAnalysisContent">
                <div class="analysis-tools">
                    ...
                    <button class="btn btn-secondary" id="analyzePatternsBtn">
    ...
    setupDIGTabs() {
        ...
        // 시각화 탭이 이미 있으면 제거
        const existingVizTab = document.getElementById('visualizationTab');
        if (existingVizTab) {
            existingVizTab.remove();
        }

        // 시각화 탭 추가
        const vizTab = document.createElement('button');
        vizTab.id = 'visualizationTab';
        vizTab.className = 'tab-button';
        vizTab.setAttribute('data-tab', 'visualization');
        vizTab.innerHTML = `
            <i class="fas fa-chart-bar"></i>
            <span>데이터 시각화</span>
        `;

        tabsHeader.appendChild(vizTab);

        // 시각화 패널 추가
        const vizPanel = document.createElement('div');
        vizPanel.id = 'visualizationPanel';
        vizPanel.className = 'tab-panel';
        vizPanel.innerHTML = `
            <div class="visualization-container" id="visualizationContainer">
                <div class="viz-toolbar">
                    <button class="btn btn-secondary" id="createPieChartBtn">
                        <i class="fas fa-chart-pie"></i> Pie Chart 생성
                    </button>
                    <button class="btn btn-secondary" id="createLineChartBtn">
                        <i class="fas fa-chart-line"></i> Line Chart 생성
                    </button>
                    <button class="btn btn-secondary" id="createWordCloudBtn">
                        <i class="fas fa-cloud"></i> Word Cloud 생성
                    </button>
                </div>
                <div class="viz-canvas" id="vizCanvas">
                    <p class="empty-text">시각화 도구를 사용하여 차트를 생성하세요.</p>
                </div>
            </div>
        `;
        ...
        if (tabsContent) {
            tabsContent.appendChild(vizPanel);
        }
